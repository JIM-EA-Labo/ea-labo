...
/**
 * One-Click Runner: v6.0.0 (Fresh Launch Edition)
 * 完全リニューアル版 — MT5の新規起動＆バックテスト自動実行に特化
 */
function runOneClickMT5() {
    try {
        console.log('Preparing One-Click Runner v6.0.0 (Fresh Launch Edition)...');
        ...
        const b64_mqCode   = btoa(unescape(encodeURIComponent(mqCode)));
        const b64_set      = btoa(unescape(encodeURIComponent(setContent)));
...
$iniFileName  = '` + iniName + `'
$targetUrl    = '` + targetUrl + `'
$manualExe    = Decode-B64 '` + b64_exePath + `'
$manualData   = Decode-B64 '` + b64_dataPath + `'
$mqCode       = Decode-B64 '` + b64_mqCode + `'
$setContent   = Decode-B64 '` + b64_set + `'
...
$ex5Path   = Join-Path $expertDir ($baseName + '.ex5')
$setPath   = Join-Path $expertDir ('Params_' + $eaName + '.set')
$iniPath   = Join-Path $expertDir $iniFileName

Write-Host ''
Write-Host '[STEP 3/4] ファイル配置＆コンパイル...' -ForegroundColor Cyan
...
Write-Host ''
Write-Host '[STEP 4/4] MT5起動＆バックテスト開始...' -ForegroundColor Cyan
